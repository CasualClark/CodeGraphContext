...
def _get_dart_package_path(package_name: str) -> Optional[str]:
    """
    Resolve a Dart/Flutter package to its local path.
    Priority: .dart_tool/package_config.json -> PUB_CACHE fallback.
    """
    # 1) Walk up to find a package_config.json (project-local truth)
    for base in [Path.cwd(), *Path.cwd().parents]:
        cfg = base / ".dart_tool" / "package_config.json"
        if cfg.exists():
            try:
    ...
    # 2) Fallback: PUB_CACHE (may be overridden via env)
    pub_cache = os.environ.get("PUB_CACHE")
    if pub_cache:
        for d in Path(pub_cache).rglob(f"{package_name}-*"):
            if d.is_dir() and (d / "lib").exists():
            ...
            if group_path:
                ...
                if group_id_full:
                    ...
                    if package_path.exists():
                        ...
                        version_dirs = [d for d in package_path.iterdir() if d.is_dir()]
                        if version_dirs:
                            latest_version = sorted(version_dirs, key=lambda x: x.name)[
                                -1
                            ]
                            # Gradle stores files in hash subdirectories
                            hash_dirs = [
                                d for d in latest_version.iterdir() if d.is_dir()
                            ]
                            if hash_dirs:
...
def _get_ruby_package_path(package_name: str) -> Optional[str]:
    ...
    try:
        ...
        result = subprocess.run(
            ["gem", "which", package_name], capture_output=True, text=True, timeout=5
        )
        if result.returncode == 0 and result.stdout.strip():
...
def _get_go_package_path(package_name: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            cp4 = subprocess.run(
                ["go", "env", "GOROOT"], capture_output=True, text=True, timeout=5
            )
            if cp4.returncode == 0:
            ...
            if cp5.returncode == 0:
                ...
                if gopath:
                    ...
                    if gopath_lib_path.exists() and gopath_lib_path.is_dir():
                        debug_log(
                            f"Found Go package in GOPATH {package_name} at {gopath_lib_path}"
                        )
