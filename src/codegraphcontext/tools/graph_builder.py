...
class TreeSitterParser:
    ...
    def parse(self, file_path: Path, is_dependency: bool = False, **kwargs) -> Dict:
        ...
        if self.language_specific_parser:
            return self.language_specific_parser.parse(
                file_path, is_dependency, **kwargs
            )
        else:
...
class GraphBuilder:
    ...
    def create_schema(self):
        ...
        with self.driver.session() as session:
            try:
                ...
                session.run(
                    "CREATE CONSTRAINT function_unique IF NOT EXISTS FOR (f:Function) REQUIRE (f.name, f.file_path, f.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT class_unique IF NOT EXISTS FOR (c:Class) REQUIRE (c.name, c.file_path, c.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT trait_unique IF NOT EXISTS FOR (t:Trait) REQUIRE (t.name, t.file_path, t.line_number) IS UNIQUE"
                )  # Added trait constraint
                session.run(
                    "CREATE CONSTRAINT interface_unique IF NOT EXISTS FOR (i:Interface) REQUIRE (i.name, i.file_path, i.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT macro_unique IF NOT EXISTS FOR (m:Macro) REQUIRE (m.name, m.file_path, m.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT variable_unique IF NOT EXISTS FOR (v:Variable) REQUIRE (v.name, v.file_path, v.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT module_name IF NOT EXISTS FOR (m:Module) REQUIRE m.name IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT struct_cpp IF NOT EXISTS FOR (cstruct: Struct) REQUIRE (cstruct.name, cstruct.file_path, cstruct.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT enum_cpp IF NOT EXISTS FOR (cenum: Enum) REQUIRE (cenum.name, cenum.file_path, cenum.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT union_cpp IF NOT EXISTS FOR (cunion: Union) REQUIRE (cunion.name, cunion.file_path, cunion.line_number) IS UNIQUE"
                )
                session.run(
                    "CREATE CONSTRAINT annotation_unique IF NOT EXISTS FOR (a:Annotation) REQUIRE (a.name, a.file_path, a.line_number) IS UNIQUE"
                )
    ...
    # --- rewrite _pre_scan_for_imports to iterate languages instead of a single elif chain
    def _pre_scan_for_imports(self, files: list[Path]) -> dict:
        ...
        for ext, group in files_by_lang.items():
            if ext in (".py", ".ipynb"):
                from .languages import python as py

                imports_map.update(py.pre_scan_python(group, self.parsers[ext]))
            elif ext in (".js", ".jsx", ".mjs", ".cjs"):
                from .languages import javascript as js

                imports_map.update(js.pre_scan_javascript(group, self.parsers[ext]))
            elif ext in (".ts", ".tsx"):
                from .languages import typescript as ts

                imports_map.update(ts.pre_scan_typescript(group, self.parsers[ext]))
            elif ext in (".cpp", ".h", ".hpp"):
                from .languages import cpp as cpp

                imports_map.update(cpp.pre_scan_cpp(group, self.parsers[ext]))
            elif ext == ".rs":
                from .languages import rust as rust

                imports_map.update(rust.pre_scan_rust(group, self.parsers[ext]))
            elif ext == ".c":
                from .languages import c as c_lang

                imports_map.update(c_lang.pre_scan_c(group, self.parsers[ext]))
            elif ext == ".java":
                from .languages import java as java

                imports_map.update(java.pre_scan_java(group, self.parsers[ext]))
            elif ext == ".rb":
                from .languages import ruby as rb

                imports_map.update(rb.pre_scan_ruby(group, self.parsers[ext]))
            elif ext == ".dart":
                from .languages import dart as dart_lang

                imports_map.update(dart_lang.pre_scan_dart(group, self.parsers[ext]))
        return imports_map
    ...
    def _create_function_calls(self, session, file_data: Dict, imports_map: dict):
        ...
        for call in file_data.get("function_calls", []):
            ...
            if call.get("inferred_obj_type"):
                ...
                lookup_name = (
                    call["full_name"].split(".")[0]
                    if "." in call["full_name"]
                    else called_name
                )
    ...
    def parse_file(
        self, repo_path: Path, file_path: Path, is_dependency: bool = False
    ) -> Dict:
        """Parses a file with the appropriate language parser and extracts code elements."""
        parser = self.parsers.get(file_path.suffix)
        if not parser:
            warning_logger(
                f"No parser found for file extension {file_path.suffix}. Skipping {file_path}"
            )
            return {
        ...
        debug_log(
            f"[parse_file] Starting parsing for: {file_path} with {parser.language_name} parser"
        )
        try:
            if parser.language_name == "python":
                is_notebook = file_path.suffix == ".ipynb"
                file_data = parser.parse(
                    file_path, is_dependency, is_notebook=is_notebook
                )
            else:
            ...
            error_logger(
                f"Error parsing {file_path} with {parser.language_name} parser: {e}"
            )
    ...
    async def build_graph_from_path_async(
        ...
        try:
            ...
            if spec:
                files = [
                    f for f in files if not spec.match_file(str(f.relative_to(path)))
                ]
            if job_id:
